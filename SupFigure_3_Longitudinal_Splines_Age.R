#------------------Longitudinal spline fits depending on age-----------------#
# Script to compute spline fits for longitudinal retinal layer thickness
# depending on age.
...
# The results are presented in Supplementary Figure 3.
    ...
    for(knots in knotrange){
      for(degree in degreerange){
        ...
        if(mse < besterror){
          ...
          # train model on
          model <- lme4::lmer(y ~ bs(df_$x,knots=knots_val,degree=degree) + sex + (1|id),data=df_)
...
show_plot <- function(outcome){
  if(outcome == "Latenz"){
    best_fit <- spline_cv_fits_lmer(data$Age_Latency,
                                    data[,outcome],
                                    data$Geschlecht,
                                    data$ID,
                                    data$Verlaufsform)
  }else{
    best_fit <- spline_cv_fits_lmer(data$Age,
                                    data[,outcome],
                                    data$Geschlecht,
                                    data$ID,
                                    data$Verlaufsform)
  ...
  #-- Plot the result
  plt <- ggplot() +
    geom_point(aes(data$Age,data[,outcome],shape=as.factor(data$Verlaufsform),color=as.factor(data$Verlaufsform)),size=0.5)+
    geom_line(aes(data$Age,data[,outcome],group=as.factor(data$ID_Eye),color=as.factor(data$Verlaufsform)),size=0.45)+
    geom_line(aes(best_fit$x,best_fit$.fitted, group=best_fit$Group, color=as.factor(best_fit$Group)), size=1.3) +
    labs(x = "Age in years", y=outcome, color="Disease Course", group="Disease Course", shape="Disease Course")+
    theme_classic()
...
pdf("supfig3.pdf",width=20,height=12)
